...
hailo_status dump_detected_object(const HailoDetectionPtr &detection, std::ofstream &detections_file)
{
    ...
    detections_file << "Detection object Xmax:          " << bbox.xmax() * YOLACT_IMAGE_WIDTH << "\n";
    detections_file << "Detection object Xmin:          " << bbox.xmin() * YOLACT_IMAGE_WIDTH << "\n";
    detections_file << "Detection object Ymax:          " << bbox.ymax() * YOLACT_IMAGE_HEIGHT << "\n";
    detections_file << "Detection object Ymin:          " << bbox.ymin() * YOLACT_IMAGE_HEIGHT << "\n"
                    << std::endl;
...
{    
    cv::resize(from_image, resized_image, cv::Size(YOLACT_IMAGE_WIDTH, YOLACT_IMAGE_HEIGHT), 0, 0, cv::INTER_AREA);
...
{   
    ...
    if(Source==SourceType::images)
    {
        ...
        auto status = get_images(source_path ,input_images, INPUT_FILES_COUNT, YOLACT_IMAGE_WIDTH, YOLACT_IMAGE_HEIGHT);
        if (HAILO_SUCCESS != status)
        {
