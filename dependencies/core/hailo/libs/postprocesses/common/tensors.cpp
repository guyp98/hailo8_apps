...
using namespace common;
    ...
    xt::xarray<uint8_t> common::get_xtensor(HailoTensorPtr &tensor)
    {
    ...
    xt::xarray<uint16_t> common::get_xtensor_uint16(HailoTensorPtr &tensor)
    {
        // Adapt a HailoTensorPtr to an xarray (quantized)
    ...
    xt::xarray<float> common::get_xtensor_float(HailoTensorPtr &tensor)
    {
        // Adapt a HailoTensorPtr to an xarray (quantized)
     ...
     */
    std::vector<HailoTensorPtr> common::get_tensor_values(const std::map<std::string, HailoTensorPtr> &tensors)
    {
